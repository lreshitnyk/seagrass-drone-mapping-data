# Time series of eelgrass (*Zostera marina*) meadow area derived from remotely piloted aerial systems (RPAS, or drone) surveys, Central Coast, British Columbia
...
Drone surveys are conducted during low-tide windows at long-term Hakai Institute monitoring sites. High-resolution RGB orthomosaics are used to delineate eelgrass extent using object-based image analysis (OBIA) and manual GIS methods, with a minimum mapping unit of 2 m². Outputs are reviewed by trained analysts and delivered as vector polygon features in NAD83 UTM Zone 9N.
...
| Superstition        | 51.894102°  | -128.234699°  | 
...
NOTE: Coverage was incomplete for Koeye in 2019 and McMullin North in 2022. Survey coverage AOI polygons are provided for these surveys.

Data dictionaries for each layer are provided as CSV files in `docs/`.

## Methods

RPAS surveys are conducted annually during morning low tides (winds < 19 kt, no rain) using a small, commercially available drone (DJI Phantom 3 Pro, DJI Phantom 4 Pro, DJI Mavic 3E). Imagery is collected following automated grid flight plans (Pix4D Capture App, Map Pilot, or DJI Go 4) to ensure complete site coverage with sufficient photo overlap. Flight altitude is set to maximise detail and meet ground sampling distance (GSD) specifications (typically <5 cm). 

Orthomosaics are generated using a Structure from Motion (SfM) workflow (Agisoft Metashape Pro) and georeferenced using ground control points collected during the survey or co-registered to a reference orthomosaic.

Eelgrass extent is delineated using a combination of object-based image analysis (OBIA) in eCognition Developer 9 and manual delineation in ArcMap (v10.8). Segmentation outputs are imported into ArcGIS, where objects are classified as eelgrass by a trained analyst based on photo-interpretive characteristics (e.g. colour, texture). Where segmentation performs poorly, segments are removed and redrawn manually. Where available, classified towed underwater video data are used to aid image interpretation of subtidal eelgrass boundaries. A minimum mapping unit (MMU) of 2 m² is applied for patches and 4 m² is applied for minimum hole size within a bed or meadow. All patches within 1 m of the closest patch are aggregated. For further details on factors affecting mapping confidence, see Nahirnick et al. (2018).

Areal extent data are provided as vector polygon features in NAD83 UTM Zone 9N, clipped to each site area of interest (AOI) to ensure consistent survey boundaries are compared over time, and published to a geodatabase. NOTE: Coverage was incomplete for Koeye in 2019 and McMullin North in 2022. Survey coverage AOI polygons are provided for these surveys.
